...
def read_deep_res(file_name):
    ...
    ORIGINAL_DATA_DIRECTORY = ORIGINAL_DATA_DIRECTORY + os.path.sep + file_name
    # print(ORIGINAL_DATA_DIRECTORY)
    ORIGINAL_DATA_DIRECTORY = pkg_resources.resource_filename(__name__, ORIGINAL_DATA_DIRECTORY)

    file = open(ORIGINAL_DATA_DIRECTORY, 'r', encoding='utf-8')
    original_data = modify(file.read())
    original_words = get_list(original_data)

    ALL_COMP_DIRECTORY = ALL_COMP_DIRECTORY + os.path.sep + file_name
    ALL_COMP_DIRECTORY = pkg_resources.resource_filename(__name__, ALL_COMP_DIRECTORY)

    file = open(ALL_COMP_DIRECTORY, 'r', encoding='utf-8')
    all_without_data = file.read()
    all_tags = get_list(all_without_data)

    CHEM_DIRECTORY = CHEM_DIRECTORY + os.path.sep + file_name
    CHEM_DIRECTORY = pkg_resources.resource_filename(__name__, CHEM_DIRECTORY)

    file = open(CHEM_DIRECTORY, 'r', encoding='utf-8')
    chem_data = file.read()
    chem_tags = get_list(chem_data)

    NECHEM_DIRECTORY = NECHEM_DIRECTORY + os.path.sep + file_name
    NECHEM_DIRECTORY = pkg_resources.resource_filename(__name__, NECHEM_DIRECTORY)

    file = open(NECHEM_DIRECTORY, 'r', encoding='utf-8')
